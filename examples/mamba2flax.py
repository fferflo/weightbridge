...
class Block(nn.Module):
    ...
    def __call__(self, x):
        x0 = x
        x = nn.RMSNorm(epsilon=1e-5)(x)

        x = nn.Dense(features=4 * x.shape[-1], use_bias=False)(x)
        ...
        A = self.param("A_param", nn.initializers.zeros_init(), (x.shape[-1], self.d_state), "float32")
        A = -jnp.exp(A) # c n
        D = self.param("D_param", nn.initializers.zeros_init(), (x.shape[-1],), "float32") # c

        # Compute input-dependent state space parameters
        dt_rank = math.ceil(x0.shape[-1] / 16)
        y = nn.Dense(features=dt_rank + self.d_state + self.d_state, use_bias=False)(x)
        delta, B, C = jnp.split(y, [dt_rank, dt_rank + self.d_state], axis=-1)

        delta = nn.Dense(features=x.shape[-1], use_bias=True)(delta)
        ...
        x = x * res
        x = nn.Dense(features=x0.shape[-1], use_bias=False)(x)
...
class Mamba(nn.Module):
    ...
    def __call__(self, x):
        ...
        vocab = lambda shape: self.param("vocabulary", nn.initializers.normal(stddev=0.02), shape, "float32")
        ...
        # Classifier
        x = nn.Dense(features=self.vocab_size, use_bias=False)(x)
...
params = model.init({"params": rng}, jnp.asarray(tokens[np.newaxis]))
apply = jax.jit(model.apply)

def predict(tokens, params, temperature=0.3):
    i = num_tokens
    for _ in range(10): # Predict next tokens
        logits = apply(params, jnp.asarray(tokens[np.newaxis]))[0, i - 1]
